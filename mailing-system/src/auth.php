...
function authRequireLogin(): array
{
    ...
    if ($user === null) {
        $requestUri = (string) ($_SERVER['REQUEST_URI'] ?? '/home/');
        $next = str_starts_with($requestUri, '/') && !str_starts_with($requestUri, '//')
            ? $requestUri
            : '/home/';
        header('Location: /home/login.php?next=' . rawurlencode($next));
        exit;
    }
    return $user;
}
